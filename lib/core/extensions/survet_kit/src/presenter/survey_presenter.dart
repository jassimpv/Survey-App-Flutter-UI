...
import 'package:collect/core/extensions/survet_kit/src/navigator/task_navigator.dart';
import 'package:collect/core/extensions/survet_kit/src/presenter/survey_event.dart';
import 'package:collect/core/extensions/survet_kit/src/presenter/survey_state.dart';
import 'package:collect/core/extensions/survet_kit/src/result/question_result.dart';
import 'package:collect/core/extensions/survet_kit/src/result/step_result.dart';
import 'package:collect/core/extensions/survet_kit/src/result/survey/survey_result.dart';
...
class SurveyPresenter extends Bloc<SurveyEvent, SurveyState> {
  ...
  SurveyPresenter({required this.taskNavigator, required this.onResult})
  ...
  SurveyState _handleNextStep(
    ...
    QuestionResult? questionResult = _getResultByStepIdentifier(
      nextStep.stepIdentifier,
    );
  ...
  SurveyState _handleStepBack(
    ...
    _addResult(event.questionResult);
    final Step? previousStep = taskNavigator.previousInList(
      currentState.currentStep,
    );

    if (previousStep != null) {
      QuestionResult? questionResult = _getResultByStepIdentifier(
        previousStep.stepIdentifier,
      );
  ...
  SurveyState _handleClose(
    ...
    List<StepResult> stepResults = results
        .map((e) => StepResult.fromQuestion(questionResult: e))
        .toList();
  ...
  void _addResult(QuestionResult? questionResult) {
    ...
    results.removeWhere(
      (QuestionResult result) => result.id == questionResult.id,
    );
